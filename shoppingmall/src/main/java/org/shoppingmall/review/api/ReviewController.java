...
import org.shoppingmall.review.api.dto.response.ReviewInfoResDto;
...
public class ReviewController {
    ...
    public ApiResponseTemplate<ReviewInfoResDto> reviewSave(@PathVariable("productId") Long productId,
        ...
        ReviewInfoResDto reviewResDto = reviewService.reviewSave(productId, reviewReqDto, reviewImage, principal);
    ...
    public ApiResponseTemplate<ReviewListResDto> reviewFindAll(@PathVariable("productId") Long productId) {
        ReviewListResDto reviewListResDto = reviewService.reviewFindAll(productId);
    ...
    @Operation(summary = "사용자만의 리뷰 상세 조회", description = "인증된 사용자가 자신의 리뷰를 상세 조회합니다.")
    @GetMapping("/user/{reviewId}")
    public ApiResponseTemplate<ReviewInfoResDto> reviewFindUser(@PathVariable("reviewId") Long reviewId, Principal principal) {
        ReviewInfoResDto reviewInfoResDto = reviewService.reviewDetailByUser(reviewId, principal);
        return ApiResponseTemplate.successResponse(SuccessCode.GET_SUCCESS, reviewInfoResDto);
    }

    @Operation(summary = "상품에 리뷰 수정", description = "인증된 사용자가 상품에 리뷰를 수정합니다.")
    @PutMapping(value = "/{reviewId}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponseTemplate<ReviewInfoResDto> reviewUpdate(@PathVariable("reviewId") Long reviewId,
                                                              @RequestPart ReviewUpdateReqDto reviewUpdateReqDto,
                                                              @RequestParam(value = "reviewImage", required = false) MultipartFile reviewImage,
                                                              Principal principal) throws IOException {
        ReviewInfoResDto reviewResDto = reviewService.reviewUpdate(reviewId, reviewUpdateReqDto, reviewImage, principal);
