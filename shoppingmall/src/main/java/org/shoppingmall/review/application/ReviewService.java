...
public class ReviewService {
    ...
    // 리뷰 등록
    public ReviewInfoResDto reviewSave(Long productId, ReviewReqDto reviewReqDto
    ...
    // 모든 리뷰 조회
    public ReviewListResDto reviewFindAll(Long productId) {
        List<Review> reviews = reviewRepository.findByProductId(productId);
        List<ReviewInfoResDto> reviewResDtos = reviews.stream()
    ...
    // 사용자 리뷰 상세 조회
    public ReviewInfoResDto reviewDetailByUser(Long reviewId, Principal principal) {
        User user = entityFinder.getUserFromPrincipal(principal);

        Review review = entityFinder.getReviewById(reviewId);
        if(!review.getUser().getId().equals(user.getId())) {
            throw new CustomException(ErrorCode.NO_AUTHORIZATION_EXCEPTION, ErrorCode.NO_AUTHORIZATION_EXCEPTION.getMessage());
        }
    ...
    // 리뷰 수정
    public ReviewInfoResDto reviewUpdate(Long reviewId, ReviewUpdateReqDto reviewUpdateReqDto, MultipartFile reviewImage, Principal principal) throws IOException {
